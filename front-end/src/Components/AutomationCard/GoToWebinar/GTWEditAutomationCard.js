...
function GTWEditAutomationCard({ setShowAutomationCard, item }) {
  ...
  const [aweberListId, setAweberListId] = useState(item.AppName ==="GTWToAweber" ? item.ListId : "");
  const [listId, setListId] = useState(item.AppName ==="GTWToBrevo" ? item.ListId : "");
  const [campaignListId, setCampaignListId] = useState(item.AppName ==="GTWToGetresponse" ? item.ListId: "");
 ...
 console.log(aweberListId)
  ...
  const gettingCampaignLists = async () => {
    await axios
      .get(
        `http://24.199.76.74:5000/getresponse/api/get/campaign?email=${user.email}`,
        {
          headers: headers,
        }
      )
  ...
  const handleStartAutomation = async () => {
   ...
   console.log(campaignListId)
    ...
    await axios
      .post(
        `http://24.199.76.74:5000/gotowebinar/api/edit/automation?email=${user.email}`,
        body,
        {
          headers: headers,
        }
...
export default GTWEditAutomationCard;
