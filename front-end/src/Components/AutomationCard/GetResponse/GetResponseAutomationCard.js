  ...
  const handleStartAutomation = async () => {
    ...
    if (operation == 1) {
      const response = await axios
        .post(
          `https://backend.connectsyncdata.com:5000/getresponse/api/start/automation?email=${user.email}`,
          body,
          {
            headers: headers,
          }
   ...
   else if(operation == 2)
    {
      const response = await axios
      .post(
        `https://backend.connectsyncdata.com:5000/getresponse/api/start/del/automation?email=${user.email}`,
        body,
        {
          headers: headers,
        }
